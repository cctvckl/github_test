...
public final class MobileHelper {
    ...
    @Config(resetable = true)
    private static String paySmsBizNo = "000002";
    @Config(resetable = true)
    private static String paySmsBizKey = "kdjsuioueoiruoui";
    @Config(resetable = true)
...
//            // 13xx 15xx 18xx 14xx 17
//            Pattern p = Pattern.compile("^((13[0-9])|(15[0-9])|(18[0-9])|(14[0-9]))\\d{8}$");
//            Matcher m = p.matcher(mobileNo);
//            result = m.matches();
//        }
       
        return  ValidateUtil.isValidMobilePhone(mobileNo);
    }
    ...
    private static String buildComplelledUrl(String mobileNo, String content) {
        ...
        params.put("bizNo", paySmsBizNo);
        params.put("mobile", mobileNo);
        params.put("content", content);
        String signMsg = SignatureHelper.sign(params, paySmsBizKey, "GBK");// 一定使用GBK编码方式处理
        return new StringBuilder(paySmsServerUrlRoot).append("sendsms?version=").append(URICodec.encode(paySmsVersion)).append("&pageCharset=").append(URICodec.encode("GBK")).append("&bizNo=")
                .append(URICodec.encode(paySmsBizNo)).append("&mobile=").append(mobileNo).append("&content=").append(URICodec.encode(content)).append("&signMsg=").append(signMsg).toString();
    }
